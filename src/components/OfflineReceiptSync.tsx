...
const OFFLINE_GENERATE_RECEIPTS_KEY = 'offline_report_receipts';
...
const OfflineReceiptSync = () => {
  ...
  const getSyncOfflineReceipts = async () => {
    try {
      ...
      const receiptStored = await AsyncStorage.getItem('offline_report_receipts');

      if (!stored && !receiptStored) {
        Dialog.show({
      ...
      const receiptsStored = receiptStored ? JSON.parse(receiptStored) : [];
      setLeadReports(receipts);
      setReceipts(receiptsStored)
    } catch (e) {
      console.log('Sync error', e);
    }
  };

  console.log("Receipts from offline page", receipts)
            ...
            {receipts
              .filter(item => {
                ...
                return (
                  ...
                  item?.ticket_no?.toString().includes(text)
                );
              })
              .map((item, index) => (
                <Pressable key={`report-${index}`} style={styles.card}>
                  <View style={styles.headerRow}>
                    <View>
                      <Text style={styles.name}>
                        {item?.userName || 'N/A'}
                    ...
                    <View style={styles.rightSection}>
                      ...
                      <Text style={[styles.amount, { paddingTop: 4 }]}>
                        ₹ {item.amount}
                      </Text>
                    </View>
                  </View>
